...
- 집계: aspect + category별로 POS/NEG/NEU 카운트 + 비율 계산 (Python 로직)
...
1. aste_results에서 aspect별 감성 집계 (Python, LLM 호출 없음)
   - key: category, value: `{"POS": n, "NEG": n, "NEU": n, "total": n, "neg_ratio": float}`
...
def aggregate(triples: list) -> dict:
    stats = defaultdict(lambda: {"POS": 0, "NEG": 0, "NEU": 0, "total": 0})
    for triple in triples:
        cat = triple["aspect"]
        stats[cat][triple["sentiment"]] += 1
        stats[cat]["total"] += 1
    for cat in stats:
